...
class EngagementPredictor {
  static const double ENGAGEMENT_BOOST_WEIGHT = 0.15;
  ...
  static Future<double> predictEngagementScore(EventModel event) async {
    try {
      ...
      Map<String, dynamic> engagementHistory = await _getUserEngagementHistory(user.uid);
      
      // Calculate engagement score based on multiple factors
      double categoryEngagement = _calculateCategoryEngagement(event, engagementHistory);
      double timeEngagement = _calculateTimeEngagement(event, engagementHistory);
      double locationEngagement = _calculateLocationEngagement(event, engagementHistory);
      double socialEngagement = _calculateSocialEngagement(event, engagementHistory);
      
      // Weighted average of engagement factors
      double engagementScore = (categoryEngagement * 0.4) +
                             (timeEngagement * 0.25) +
  ...
  static Future<Map<String, dynamic>> _getUserEngagementHistory(String userId) async {
    try {
      ...
      for (var doc in interactions.docs) {
        ...
        String interactionType = data['interactionType'] ?? 'view';
        ...
        if (eventDoc.exists) {
          EventModel event = EventModel.fromJson(eventDoc.data() as Map<String, dynamic>);
          ...
          String locationZone = _getLocationZone(event.latitude, event.longitude);
          ...
          String socialLevel = _getSocialLevel(event.issuedTickets, event.maxTickets);
  ...
  static double _calculateCategoryEngagement(EventModel event, Map<String, dynamic> history) {
  ...
  static double _calculateTimeEngagement(EventModel event, Map<String, dynamic> history) {
    if (history['totalInteractions'] == 0) return 0.5;
  ...
  static double _calculateLocationEngagement(EventModel event, Map<String, dynamic> history) {
    if (history['totalInteractions'] == 0) return 0.5;
    
    String eventLocationZone = _getLocationZone(event.latitude, event.longitude);
    int locationInteractions = history['locationInteractions'][eventLocationZone] ?? 0;
    
    return locationInteractions / history['totalInteractions'];
  }

  /// Calculate social engagement score
  static double _calculateSocialEngagement(EventModel event, Map<String, dynamic> history) {
    if (history['totalInteractions'] == 0) return 0.5;
    
    String eventSocialLevel = _getSocialLevel(event.issuedTickets, event.maxTickets);
    int socialInteractions = history['socialInteractions'][eventSocialLevel] ?? 0;
  ...
  static String _getLocationZone(double latitude, double longitude) {
    ...
    int latZone = (latitude * 10).round();
    int lonZone = (longitude * 10).round();
  ...
  static double applyEngagementBoost(double baseScore, double engagementScore) {
    return baseScore + (engagementScore * ENGAGEMENT_BOOST_WEIGHT);
  }

  /// Track user interaction with an event
  static Future<void> trackInteraction(String eventId, String interactionType) async {
    try {
      final user = FirebaseAuth.instance.currentUser;
      if (user == null) return;
