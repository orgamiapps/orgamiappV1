...
class AccessListManagementWidget extends StatefulWidget {
  ...
  @override
  State<AccessListManagementWidget> createState() => _AccessListManagementWidgetState();
}

class _AccessListManagementWidgetState extends State<AccessListManagementWidget> {
  ...
  Future<void> _search() async {
    ...
    try {
      final users = await FirebaseFirestoreHelper().searchUsers(searchQuery: q, limit: 20);
  ...
  Future<void> _addToAccess(String userId) async {
    ...
    try {
      await FirebaseFirestore.instance
          ...
          .update({'accessList': FieldValue.arrayUnion([userId])});
  ...
  Future<void> _removeFromAccess(String userId) async {
    ...
    try {
      await FirebaseFirestore.instance
          .collection(EventModel.firebaseKey)
          .doc(widget.eventModel.id)
          .update({'accessList': FieldValue.arrayRemove([userId])});
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                const Expanded(
                  child: Text('Manage Access', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600)),
                ),
                IconButton(
                  ...
                  itemBuilder: (context, i) {
                    ...
                    return ListTile(
                      ...
                      trailing: _updating
                          ...
                          : ElevatedButton.icon(
                              onPressed: inAccess ? () => _removeFromAccess(u.uid) : () => _addToAccess(u.uid),
            ...
            const SizedBox(height: 12),
            const Text('Current Access', style: TextStyle(fontWeight: FontWeight.w600)),
            const SizedBox(height: 8),
            SizedBox(
              height: 120,
              child: StreamBuilder<DocumentSnapshot>(
                ...
                builder: (context, snapshot) {
                  ...
                  if (snapshot.hasData && snapshot.data!.exists) {
                    ...
                    final List<dynamic>? arr = data['accessList'] as List<dynamic>?;
                    if (arr != null) {
                      // Update local cache
                      arr.map((e) => e.toString()).toList();
                    }
                  }
                  if (list.isEmpty) {
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (context, i) {
                      final uid = list[i];
                      return ListTile(
                        leading: const CircleAvatar(child: Icon(Icons.lock_open)),
                        title: Text(uid),
                        trailing: _updating
                            ? const SizedBox(
                                width: 20,
