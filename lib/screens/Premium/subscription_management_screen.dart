...
class _SubscriptionManagementScreenState extends State<SubscriptionManagementScreen> {
  ...
  Widget _buildSubscriptionOverview(subscription, SubscriptionService subscriptionService) {
    return Container(
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildDetailRow('Price', '${subscription.formattedPrice}/${subscription.interval}'),
          _buildDetailRow('Started', _formatDate(subscription.createdAt)),
          _buildDetailRow('Current Period', 
            '${_formatDate(subscription.currentPeriodStart)} - ${_formatDate(subscription.currentPeriodEnd)}'),
          if (subscription.cancelledAt != null)
            _buildDetailRow('Cancelled', _formatDate(subscription.cancelledAt!)),
        ],
      ),
    );
  }

  Widget _buildManagementActions(subscription, SubscriptionService subscriptionService) {
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(
  ...
  Future<void> _showCancelConfirmation(SubscriptionService subscriptionService) async {
  ...
  Future<void> _reactivateSubscription(SubscriptionService subscriptionService) async {
    setState(() {
      _isLoading = true;
    });
