...
class NFCBadgeService {
  ...
  Future<NFCBadgeReadResult> startNFCBadgeReading({
    ...
    required Duration timeout = const Duration(seconds: 30),
  }) async {
    try {
      ...
      await NfcManager.instance.startSession(
        onDiscovered: (NfcTag tag) async {
          try {
            ...
            if (result.isSuccess) {
              await NfcManager.instance.stopSession(alertMessage: 'Ticket activated successfully!');
            } else {
              await NfcManager.instance.stopSession(errorMessage: result.error ?? 'Failed to activate ticket');
            }
            
          } catch (e) {
            debugPrint('Error processing NFC tag: $e');
            if (!completer.isCompleted) {
              completer.complete(NFCBadgeReadResult.error('Error processing badge: $e'));
            }
            await NfcManager.instance.stopSession(errorMessage: 'Error processing badge');
          }
        },
      );
    ...
    try {
      ...
      if (ndef != null) {
        ...
        if (ndefMessage.records.isNotEmpty) {
          final result = await _processNDEFRecords(ndefMessage.records, eventId, organizerUid);
          if (result.isSuccess) {
            return result;
          }
        }
    ...
    for (final record in records) {
      try {
        ...
        if (record.typeNameFormat == NdefTypeNameFormat.nfcWellknown &&
          ...
          if (record.type.first == 0x54) { // 'T' for text record
          ...
          if (record.type.first == 0x55) { // 'U' for URI record
  ...
  String? _extractTagUID(NfcTag tag) {
    try {
      // Try different tag technologies to get UID
      ...
      if (nfcB != null) {
        return nfcB.identifier.map((e) => e.toRadixString(16).padLeft(2, '0')).join();
      }

      final nfcF = NfcF.from(tag);
      if (nfcF != null) {
        return nfcF.identifier.map((e) => e.toRadixString(16).padLeft(2, '0')).join();
      }

      final nfcV = NfcV.from(tag);
      if (nfcV != null) {
        return nfcV.identifier.map((e) => e.toRadixString(16).padLeft(2, '0')).join();
      }
  ...
  Future<void> stopNFCSession({String? message}) async {
    try {
      await NfcManager.instance.stopSession(
        alertMessage: message ?? 'NFC session stopped',
      );
    } catch (e) {
        ...
        onDiscovered: (NfcTag tag) async {
          try {
            ...
            final ndefMessage = NdefMessage([
              NdefRecord.createText(badge.badgeQrData),
            ]);

            await ndef.write(ndefMessage);
            await NfcManager.instance.stopSession(alertMessage: 'Badge data written successfully!');
            completer.complete(true);

          } catch (e) {
            debugPrint('Error writing to NFC tag: $e');
            await NfcManager.instance.stopSession(errorMessage: 'Failed to write badge data');
            completer.complete(false);
          }
        },
      );
