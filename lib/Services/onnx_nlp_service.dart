...
class OnnxNlpService {
  ...
  Future<void> _loadMetadata() async {
    try {
      final String metadataJson = await rootBundle.loadString(
        'assets/models/intent_metadata.json',
      );
  ...
  Future<void> _initializeDatabase() async {
    ...
    _database = await openDatabase(
      ...
      onCreate: (db, version) async {
        ...
        await db.execute(
          'CREATE INDEX IF NOT EXISTS idx_location ON events(location_lat, location_lng)',
        );
  ...
  Future<Map<String, dynamic>> parseQuery(String query) async {
    ...
    try {
      ...
      Logger.debug(
        '🤖 AI parsed query: ${intent['categories']} | Near: ${intent['nearMe']}',
      );
  ...
  /// Process model output to extract intent
  Map<String, dynamic> _processModelOutput(
  ...
  Map<String, dynamic> _detectLocationIntent(
    String query,
    ...
    return {'nearMe': nearMe, 'radiusKm': radiusKm};
  }
  ...
  Map<String, String> _detectTimeIntent(String query) {
    ...
    if (queryLower.contains('today')) {
    ...
    } else if (queryLower.contains('this weekend') ||
        queryLower.contains('this week end')) {
      ...
      final startOfNextWeek = nextWeek.subtract(
  ...
  double _calculateConfidence(List<dynamic>? logits) {
    ...
    for (final logit in logits) {
      sumExp += math.exp((logit as num).toDouble() - maxLogit);
    }
  ...
  Future<List<Map<String, dynamic>>> queryEvents({
    ...
    final rawDateRange = intent['dateRange'];
    final dateRange = rawDateRange is Map
        ? rawDateRange.map(
    ...
    if (intent['nearMe'] == true && userLat != null && userLng != null) {
      ...
      final lngDelta =
          radiusKm / (111.0 * math.cos(userLat.abs() * math.pi / 180));
