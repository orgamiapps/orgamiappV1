...
async function auth(method, body) {
  const response = await fetch(`${authApi}/accounts:${method}?key=emulator-key`, {method: "POST", headers: {"content-type": "application/json"}, body: JSON.stringify(body)}); assert.equal(response.ok, true, await response.text()); return response.json();
}
